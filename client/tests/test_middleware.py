...
def test_signature_check_middleware_unknown_client(admin_client, settings, sender_id):
    settings.MIDDLEWARE = SIGNATURE_CHECK_REQUIRED_MIDDLEWARE
...
def test_signature_check_middleware_valid_client(admin_client, settings):
    ...
    response = admin_client.get(url, {}, HTTP_X_SIGNATURE=headers[signer.header_name])
