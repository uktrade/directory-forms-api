...
def test_generic_form_submission_submit_database_only(mock_delay, api_client):
    ...
    response = api_client.post(reverse("api:submission"), data=payload, format="json")
...
def test_email_action(mock_delay, api_client, email_action_payload):
    response = api_client.post(
        reverse("api:submission"), data=email_action_payload, format="json"
    )
...
def test_zendesk_action(mock_delay, api_client, zendesk_action_payload, settings):
    response = api_client.post(
        reverse("api:submission"), data=zendesk_action_payload, format="json"
    )
    expected_payload = {
        **zendesk_action_payload["data"],
        "ingress_url": zendesk_action_payload["meta"]["ingress_url"],
        "_sort_fields_alphabetically": zendesk_action_payload["meta"][
...
def test_gov_notify_email_action(
    ...
    response = api_client.post(
        reverse("api:submission"), data=gov_notify_email_action_payload, format="json"
    )
...
def test_gov_notify_action_old_constant(
    ...
    response = api_client.post(
        reverse("api:submission"), data=gov_notify_action_payload_old, format="json"
    )
...
def test_gov_notify_letter_action(
    ...
    response = api_client.post(
        reverse("api:submission"), data=gov_notify_letter_action_payload, format="json"
    )
...
def test_pardot_action(mock_delay, api_client, pardot_action_payload):
    response = api_client.post(
        reverse("api:submission"), data=pardot_action_payload, format="json"
    )
...
@pytest.mark.django_db
@mock.patch("submission.tasks.send_gov_notify_email.delay")
def test_email_action_rate_limit_not_exceeded(
    mock_email, api_client, gov_notify_email_action_payload, settings
):
...
@pytest.mark.django_db
@mock.patch("submission.tasks.send_gov_notify_email.delay")
def test_email_action_rate_limit_exceeded(
    mock_email, api_client, gov_notify_email_action_payload, settings
):
...
@pytest.mark.django_db
@mock.patch("submission.tasks.send_gov_notify_email.delay")
def test_invalid_credential_unauthorized_error_on_submission(
    api_client, gov_notify_email_action_payload, settings
):
...
@pytest.mark.django_db
@mock.patch("submission.helpers.send_email")
def test_form_submission_delete_action(
    mock_delay, mock_middleware_test_sig, api_client
):
    assert models.Submission.objects.count() == 0

    instance = factories.SubmissionFactory(
        data={"html_body": "<html><head></head><body>Hello</body></html>"}
    )
    ...
    delete_response = api_client.delete(
        reverse(
            ...
            kwargs={"email_address": instance.sender.email_address},
        ),
        data=None,
        format="json",
    )
...
class TestBulkGovNotifyEmail:
    ...
    @pytest.mark.django_db
    def test_gov_notify_bulk_email_action_success(
        self, api_client, gov_notify_bulk_email_action_payload
    ):
        assert models.Submission.objects.count() == 0
    ...
    @pytest.mark.django_db
    def test_gov_notify_bulk_email_action_fails_on_bad_input(
        self, api_client, gov_notify_bulk_email_action_payload
    ):
        assert models.Submission.objects.count() == 0
    ...
    @pytest.mark.django_db
    def test_gov_notify_bulk_email_action_fails_on_missing_email(
        self, api_client, gov_notify_bulk_email_action_payload
    ):
        assert models.Submission.objects.count() == 0

        # Malform the data payload by removing a single email entry
        del gov_notify_bulk_email_action_payload["bulk_email_entries"][0][
            "email_address"
        ]
...
def test_hcsat_submmission_success(api_client, hcsat_bulk_instance):
    ...
    assert response.status_code == 201
    assert models.Submission.objects.count() == 2
