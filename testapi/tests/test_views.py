...
def test_find_submissions_by_email(api_client_enabled_test_api):
    ...
    response = api_client_enabled_test_api.post(
        reverse("api:submission"), data=payload, format="json"
    )
    ...
    response = api_client_enabled_test_api.get(
        reverse(
            "testapi:submissions_by_email", kwargs={"email_address": "foo@bar.com"}
        ),
        format="json",
    )
...
@pytest.mark.django_db
def test_delete_test_submissions_returns_403_when_unauthenticated(
    api_client_test_white_listed,
):
...
def test_delete_test_senders(api_client_enabled_test_api):
    SenderFactory.create_batch(
        3, email_address=Sequence(lambda n: f"test+{n}@directory.uktrade.io")
    )
    response = api_client_enabled_test_api.delete(
...
@pytest.mark.django_db
def test_delete_test_senders_returns_404_when_no_test_senders(
    api_client_enabled_test_api,
):
    response = api_client_enabled_test_api.delete(
        reverse("testapi:delete_test_senders")
...
@pytest.mark.django_db
def test_delete_test_senders_returns_404_with_disabled_testapi(
    api_client_disabled_testapi,
):
...
@pytest.mark.django_db
def test_delete_test_senders_returns_403_when_unauthenticated(
    api_client_test_white_listed,
):
    response = api_client_test_white_listed.delete(
        reverse("testapi:delete_test_senders"),
...
@pytest.mark.django_db
def test_delete_test_senders_and_submissions_with_disabled_testapi(
    api_client_disabled_testapi,
):
    SubmissionFactory.create_batch(
        3,
